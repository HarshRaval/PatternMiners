...
public class GenerateProductCSV {
	...
	public HashSet<String> generateCSVFromXML(String fileName,
			String csvFileName) {
		...
		try {
			...
			for (int i = 0; i < nList.getLength(); i++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					pWriter.print(element.getElementsByTagName("sku").item(0)
							.getTextContent().toString());
					pWriter.print(",");
					pWriter.print(element.getElementsByTagName("productId")
							.item(0).getTextContent().toString());
					pWriter.print(",");

					String productName = replaceSpecialCharacters(element
					...
					pWriter.print(",");
					productNameToWords = wc.loadProductNameWords(productName,
							productNameToWords);
					// System.out.println(productNameToWords.toString());

					String formatvalue = processStringValues(element
							.getElementsByTagName("format").item(0)
							.getTextContent().toString(), stopwords);
					...
					String shortDesc = replaceSpecialCharacters(element
							.getElementsByTagName("shortDescription").item(0)
							.getTextContent().toString());
					...
					pWriter.print(checkNodeExistence(element, element
							.getElementsByTagName("manufacturer").getLength(),
							"manufacturer"));
					pWriter.print(",");

					pWriter.print(checkDetailsNode(element, element
							.getElementsByTagName("details").getLength(),
							"genre"));
					pWriter.print(",");
					pWriter.print(checkNodeExistence(element, element
							.getElementsByTagName("active").getLength(),
							"active"));
					pWriter.print(",");
					pWriter.print(checkNodeExistence(element, element
							.getElementsByTagName("customerReviewAverage")
							.getLength(), "customerReviewAverage"));
					pWriter.print("\n");
				}
			}
			pWriter.flush();
	...
	/*
	 * This function will process all the string values before writing them back
	 * to the product CSV file
	 */
	...
	/*
	 * This function takes the individual product element, count of details tag
	 * and the XML tag whose value needs to be returned back to the calling
	 * function
	 */
	private String checkDetailsNode(Element element, int nodeCount,
			String xmlTag) {
		if (nodeCount == 0) {
		...
		for (int i = 0; i < detailsNode.getLength(); i++) {
			...
			if (tempNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				String str = tempEl.getElementsByTagName("name").item(0)
						.getTextContent().toString();
				if (str.equals("Genre")) {
					String genreValue = tempEl.getElementsByTagName("value")
							.item(0).getTextContent().toString();
					genreValue = replaceSpecialCharacters(genreValue);
					// This value is returned when we have the detail tag as
					// well as sub-tags with name-value pair
					return genreValue;
				}
			}
		}
		// This value is returned we have a tag for detail but does not contain
		// sub-tags showing Genre information
		return genre;
	}

	/*
	 * This function is used to check whether a particular node is present
	 * inside an element It takes the element, the node count count and the XML
	 * tag which needs to be checked and returns string value that is present
	 * within the tag
	 */
	private String checkNodeExistence(Element element, int nodeCount,
			String xmlTag) {
		if (nodeCount == 0) {
			return "";
		}
		String node = element.getElementsByTagName(xmlTag).item(0)
				.getTextContent().toString();
		return node;
	}
